...
export default {
      ...
      .then(
        function (response) {
          ...
          for (let i = 0; i < this.contentsCount; i++) {
            ...
            (content.word_blank =
              word_en_begin + "_".repeat(content.word_en.length - 1)), //
              (content.phrase_quiz = content.phrase_en.replace(
                content.word_en,
                "_".repeat(content.word_en.length)
              )), //英語のフレーズのなかで問題となる部分をを'_'で置き換える
              (content.correct_answer_rate =
                (content.c_counter / content.s_counter) * 100);
  ...
  methods: {
    setRandomNum() {
      //最初は乱数にしていたが、正答率が低い順に並べることにしたので、やめた。
      if (this.isRandom) {
    ...
    updateQuizBlank() {
      ...
      this.letterLocation++;
      this.contents[this.quizNumber].word_blank =
        content.word_en.substring(0, this.letterLocation) +
        "_".repeat(content.word_en.length - this.letterLocation);
    },
    updataDatabase() {
    ...
    nextQuiz() {
      ...
      this.contents[this.quizNumber].word_blank =
        this.contents[this.quizNumber].word_en_begin +
        "_".repeat(this.contents[this.quizNumber].word_en.length - 1);
      this.setRandomNum();
      this.letterLocation = 1;
    },
    updatedCounterValue(...args) {
      //update s,c counter value from child component
